#!/usr/bin/env python3
"""Run the weekly wiki maintenance cycle.

Usage:
  python3 run_weekly.py /absolute/path/to/wiki
"""
...
def main() -> int:
    if len(sys.argv) != 2:
        print('Usage: python3 run_weekly.py /absolute/path/to/wiki', file=sys.stderr)
        return 1

    root = Path(sys.argv[1]).expanduser().resolve()
    scripts = root / 'scripts'
    steps = [
        ['python3', str(scripts / 'run_daily.py'), str(root)],
        ['python3', str(scripts / 'breakdown.py'), str(root)],
    ...
    snapshot = root / 'meta' / 'REPORTS' / f'status-weekly-{stamp}.md'

    lines = [f'# Weekly Status {stamp}', '']
    for item in results:
